...
protected:
	...
	//UFUNCTION()					// UFUNCTION이 없으면 블루프린트에서 호출 할 수 없고, 있더라도 "BlueprintCallable"이 없으면 호출 할 수없다.
	void Test_NormalFunction(); 


	// BlueprintCallable
	UFUNCTION(BlueprintCallable)			// C++로 작성한 함수를 블루프린트에서 호출 할 수 있다.
	void Test_UFunction();

	UFUNCTION(BlueprintImplementableEvent)	// C++에서 선언만 하고 구현은 블루프린트에서 하겠다.
	void Test_ImplementableFunction();		// (블루프린트 코드를 써야만 하는 상황일 때 사용. CPP에 구현하면 안됨)

	UFUNCTION(BlueprintNativeEvent)			// C++에서 선언과 구현을 했는데 블루프린트에서 재정의 할 수도 있다.(반드시 블루프린트 내에서 오버라이드를 통해 만들어야 함)
...
protected:
	...
	UPROPERTY(BlueprintReadOnly,EditAnywhere, Category = "TestActor변수|Read")	// 블루프린트 이벤트 그래프에서 읽을 수 있고 디테일창에서 편집도 가능.
	int32 Data4_2 = 42;
